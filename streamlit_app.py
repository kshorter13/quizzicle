        ...
        if game_state["status"] == "waiting":
            ...
            if quiz_mode == "instructor_paced":
                ...
                if current_q_index < total_questions:
                    ...
                    with nav_cols[0]:
                        if st.button("Previous Question", disabled=current_q_index == 0, use_container_width=True, key=f"prev_q_{current_q_index}"):
                            update_game_state(game_pin, {f"players.{player_name}.last_answered_q": player_last_answered_q - 1})
                            st.rerun()
                    with nav_cols[1]:
                        if st.button("Next Question", disabled=player_last_answered_q < current_q_index, use_container_width=True, key=f"next_q_{current_q_index}"):
                            update_game_state(game_pin, {f"players.{player_name}.last_answered_q": player_last_answered_q + 1})
                            st.rerun()
                else:
        ...
        with col_right:
            with st.container(border=True):
                if game_state["status"] == "waiting":
                    ...
                    if st.button("Start Game", disabled=not game_state.get("players"), use_container_width=True, key="start_game_btn"):
                    ...
                    if game_state.get("quiz_mode") == "instructor_paced":
                        ...
                        if st.button("Next Question", use_container_width=True, type="primary", key="next_q_host_btn"):
                        ...
                        if st.button("End Quiz Early", use_container_width=True, type="primary", key="end_quiz_btn"):
